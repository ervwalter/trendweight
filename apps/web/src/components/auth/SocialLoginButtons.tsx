...
export function SocialLoginButtons({ onSocialLogin }: SocialLoginButtonsProps) {
  return (
    <div className="space-y-3">
      <Button onClick={() => onSocialLogin("google")} variant="secondary" size="lg" className="w-full justify-start gap-4">
        <img src="/google-logo-NePEveMl.svg" alt="Google" className="h-5 w-5" />
        <span className="flex-1 text-left">Continue with Google</span>
      </Button>

      <Button onClick={() => onSocialLogin("microsoft")} variant="secondary" size="lg" className="w-full justify-start gap-4">
      ...
      <Button onClick={() => onSocialLogin("apple")} variant="secondary" size="lg" className="w-full justify-start gap-4">
