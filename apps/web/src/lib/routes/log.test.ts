...
describe("/log route", () => {
  it("sends signed-out visitors to login, remembering where they were going", async () => {
    ...
    await expectRedirect(runBeforeLoad(Route, { context: routerContext({ auth: signedOutAuth() }), location: "/log" }), {
      to: "/login",
      search: { from: "/log" },
    });
  ...
  it("only checks the profile: manual entry must work with zero connected providers", async () => {
