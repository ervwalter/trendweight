...
describe("/link route", () => {
  it("sends signed-out visitors to login, remembering where they were going", async () => {
    ...
    await expectRedirect(runBeforeLoad(Route, { context: routerContext({ auth: signedOutAuth() }), location: "/link" }), {
      to: "/login",
      search: { from: "/link" },
    });
  ...
  it("only checks the profile: a user with no providers yet must be able to reach this page", async () => {
